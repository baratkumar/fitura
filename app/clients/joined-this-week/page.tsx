...
export default function JoinedThisWeekPage() {
  ...
  const fetchClients = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/clients/joined-this-week?${params.toString()}`)
      if (response.ok) {
    ...
    <div className="px-3 sm:px-6 py-4 border-t border-gray-200 flex flex-col sm:flex-row items-center justify-between gap-4 bg-white">
      <div className="flex flex-wrap items-center gap-3 sm:gap-4">
        <p className="text-sm text-gray-600">
          ...
          <span className="font-medium">{endItem}</span> of <span className="font-medium">{total}</span> clients
        </p>
        <select
          value={limit}
          onChange={(e) => {
  ...
  if (loading && clients.length === 0) {
    return (
      <div className="container mx-auto px-4 py-10">
        <PageLoader message="Loading clients joined this week..." />
      </div>
    )
  }
    ...
    <div className="container mx-auto px-4 py-10">
      <div className="mb-8 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <div>
          ...
          <h1 className="text-3xl sm:text-4xl font-bold mb-2">Clients Joined This Week</h1>
          <p className="text-gray-600 text-sm sm:text-base">
            Registrations from Monday to Sunday this week
          </p>
        </div>
        <Link
          href="/clients"
      ...
      {total > 0 ? (
          ...
          {loading && (
            ...
            >
              <div className="flex flex-col items-center gap-3">
                <div className="w-10 h-10 border-2 border-fitura-blue border-t-transparent rounded-full animate-spin" />
                  ...
                  <tr key={client.clientId} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 sm:px-6 py-4">
                      ...
                      <div className="text-xs text-gray-500 md:hidden mt-1">
                        Joined: {client.joiningDate ? new Date(client.joiningDate).toLocaleDateString() : 'N/A'}
                      </div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-12 text-center">
          ...
          <h3 className="text-2xl font-semibold mb-2">No clients joined this week</h3>
          <p className="text-gray-500 mb-6">
            Registrations from Monday to Sunday this week will appear here
          </p>
          <Link
            href="/dashboard"
